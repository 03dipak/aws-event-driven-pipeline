...
from glue_etl.helpers import DEDUPLICATE_KEYS_WAREHOUSES,REQUIRED_COLUMNS_WAREHOUSES,ICEBERG_TABLE_WAREHOUSES,TABLE_NAME_WAREHOUSES,DEDUPLICATE_ORDERBY_WAREHOUSES,WAREHOUSES_SCHEMA,BUCKET_NAME
...
def run_etl():
    ...
    try:
        ...
        file_keys = get_new_files(TABLE_NAME_WAREHOUSES)
        for file_key in file_keys:
            try:
                df = read_csv(spark,WAREHOUSES_SCHEMA,file_key)
                df = validate_data(df,REQUIRED_COLUMNS_WAREHOUSES)
                df = deduplicate_data(df, DEDUPLICATE_KEYS_WAREHOUSES, DEDUPLICATE_ORDERBY_WAREHOUSES)
                df = df.withColumn("ingestion_timestamp", F.current_timestamp())
                write_to_iceberg(df, ICEBERG_TABLE_WAREHOUSES)
