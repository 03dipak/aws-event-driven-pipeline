...
def run_etl():
    ...
    try:
        ...
        file_keys = get_new_files(TABLE_NAME_SUPPLIERS)
        for file_key in file_keys:
            try:
                df = read_csv(spark,SUPPLIERS_SCHEMA,file_key)
                df = validate_data(df,REQUIRED_COLUMNS_SUPPLIERS)
                df = deduplicate_data(df, DEDUPLICATE_KEYS_SUPPLIERS, DEDUPLICATE_ORDERBY_SUPPLIERS)
                df = df.withColumn("ingestion_timestamp", F.current_timestamp())
                write_to_iceberg(df, ICEBERG_TABLE_SUPPLIERS)
